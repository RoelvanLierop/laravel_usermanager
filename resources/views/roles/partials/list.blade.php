    ...
    <tbody>
        ...
        <tr class="border-t">
            <td class="p-0 py-2 m-0 text-start">
                <a href="{{ route('role_read', ['id' => $role->id ] ) }}">{{ $role->name }}</a>
            </td>
            <td class="py-2" style="width:40px">&nbsp;</td>
            <td class="py-2" style="width:40px">
                <a class="rounded-xl bg-orange-400 float-right block" href="{{ route('role_read', ['id' => $role->id ] ) }}">
            ...
            <td class="py-2" style="width:40px">
                @if( $role->id > 1 )
                    <a class="rounded-xl bg-red-400 float-right block" href="{{ route('role_delete', [ 'id' => $role->id ] ) }}">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
